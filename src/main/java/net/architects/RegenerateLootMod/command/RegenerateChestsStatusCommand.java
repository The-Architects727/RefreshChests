...
public class RegenerateChestsStatusCommand {
    public static void register(CommandDispatcher<ServerCommandSource> serverCommandSourceCommandDispatcher, CommandRegistryAccess commandRegistryAccess, CommandManager.RegistrationEnvironment registrationEnvironment) {
        serverCommandSourceCommandDispatcher.register(CommandManager.literal("chests")
                .then(CommandManager.literal("regenerate").then(CommandManager.literal("status").executes(RegenerateChestsStatusCommand::run)).requires((source) -> source.hasPermissionLevel(2))));
    }
...
//                    context.getSource().getWorld().setBlockState(position, context.getSource().getWorld().getBlockState(position));
                ...
                if(CurrentProgress > PriorProgress) {
                    ...
                    context.getSource().sendFeedback(() -> Text.literal(("Progress: " + outputProgress)), false);
                }
